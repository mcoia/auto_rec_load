...
use selenium;
use seleniumProquest;
use seleniumOverdrive;
...
GetOptions(
    ...
    "job=i"              => \$runJobID,
    "testMARC=s"         => \$testMARC,
    "testMARCFilename=s" => \$testMARCFilename,
    "lockfile=s"         => \$lockfile,
...
--testMARCFilename                            [Specific marc file to be used for testing purposes.]
...
sub main
{
    ...
    # Only function calls inside main() except for logs.

    defangSpecificSource();
    defangSpecificClient();
    figurePIDFileStuff();
    checkConfig();
    ...
    initScreenShotDIR() if ($debug);

    runScrapers() if ($action eq 'run_scrapers');
    runProcessMarcJobs() if ($action eq 'run_jobs');
    runCheckILSLoaded() if ($action eq 'run_ils_check');
    runWWWActions() if ($action eq 'www_actions');
    runEmails() if ($action eq 'fire_emails');
...
sub checkConf
{
    ...
    %conf = %{$conf};
    return 1 if ($conf{"logfile"});

    # maybe do something like...
    # checkConfig() if ($conf{"logfile"});
    # and have checkConfig() return a 1. 
    # I don't like having the code terminate in different sections. 
    # We should exit here and only here. I just feel this needs more work.
    ...
    while ((my $key, my $value) = each(%all))
    {
        ...
        if (checkFolders(\%details)) # make sure that the output folders are pre-created. We expect that these are special and have external mechanism for them.
        {
            ...
            # todo: Fix the commented out code 
            {
                ...
                print "Scraping\n" if $debug;
                $source->scrape();
                # $source->processDownloadedFile('test','/mnt/evergreen/tmp/auto_rec_load/tmp/4/CustomMarcExpress.zip');
                die if $source->getError();
...
sub runTest
{
    ...
    my $cwd = getcwd();
    my $marcin = "$cwd/$testMARCFilename";
    print "marcin: [$marcin]\n";
    if (!(-e $marcin))
    {
        print "Can't test without some MARC, please provide 'test.mrc' in current working directory\n";
    }
    my $before = new Loghandler("$cwd/logs/before.txt");
    my $after = new Loghandler("$cwd/logs/after.txt");
    $before->truncFile("");
    $after->truncFile("");
...
sub getScraperJobs
{
    ...
    if (!$specific_source && !$specific_client)
    {
        ...
        my $fill = "AND source.id IN (select source from $stagingTablePrefix" . "_job WHERE type='scraper' AND run_time < (NOW() + INTERVAL 1 MINUTE) )";
...
sub deleteLockFile
{
    # note: lockfile is global  
    unlink $lockfile; # deletes the lockfile!
}
